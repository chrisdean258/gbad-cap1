...
inline void PatternTree::addLeg ( NodeId connectingnode, const int depth, 
...
void PatternTree::addExtensionLegs ( Tuple &tuple, LegOccurrences &legoccurrences ) 
{
   ...
   EdgeLabel minlabel = NOEDGELABEL;
   EdgeLabel neglect = NOEDGELABEL;
   EdgeLabel pathlowestlabel = treetuples[tuple.depth + 1 + rootpathstart].label;
   ...
   if ((candidatelegsoccurrences[pathlowestlabel].frequency >= minfreq) ||
       ((candidatelegsoccurrences[pathlowestlabel].frequency < minfreq) &&
        (candidatelegsoccurrences[pathlowestlabel].frequency > 0) &&
...
void PatternTree::addLeftLegs ( Path &path, PathLeg &leg, int &i, 
                                Depth olddepth, EdgeLabel lowestlabel, 
                                int leftend, int edgesize2 ) 
{
   ...
   for ( ; (unsigned int)i < path.legs.size () && path.legs[i]->tuple.depth <= (unsigned int)leftend; i++ ) 
   {
         ...
         while ( i2 < path.legs.size () && path.legs[i2]->tuple.depth == olddepth ) 
         {
            if ( path.legs[i2]->tuple.edgelabel == lowestlabel ) 
            {
               LegOccurrencesPtr legoccurrencesptr = join ( leg.occurrences, 
                                                            path.legs[i2]->tuple.connectingnode, 
                                                            path.legs[i2]->occurrences );
               if ( legoccurrencesptr )
                  addLeg ( path.legs[i2]->tuple.connectingnode, 
                           edgesize2 - olddepth, path.legs[i2]->tuple.edgelabel,
                           *legoccurrencesptr );
               break;
            }
            i2++;
      ...
      {
         LegOccurrencesPtr legoccurrencesptr = join ( leg.occurrences, 
                                                      path.legs[i]->tuple.connectingnode, 
                                                      path.legs[i]->occurrences );
         if ( legoccurrencesptr )
            addLeg ( path.legs[i]->tuple.connectingnode, 
                     edgesize2 - olddepth, path.legs[i]->tuple.edgelabel, 
                     *legoccurrencesptr );
      }
   }
}
...
int PatternTree::addLeftLegs ( Path &path, PathLeg &leg, Tuple &tuple, 
                               unsigned int legindex, int leftend, int edgesize2 ) 
{
   int i;

   LegOccurrencesPtr legoccurrencesptr = join ( leg.occurrences );
   if ( legoccurrencesptr )
      addLeg ( leg.tuple.connectingnode, tuple.depth, tuple.label, 
               *legoccurrencesptr );
   ...
   if ( rootpathrelations.back () == 0 && tuple.depth != maxdepth ) 
   {
      ...
      for ( i++; (unsigned int)i < legindex; i++ ) 
      {
         if ( path.legs[i]->tuple.edgelabel != lowestlabel ) 
         {
            legoccurrencesptr = join ( leg.occurrences, 
                                       path.legs[i]->tuple.connectingnode, 
                                       path.legs[i]->occurrences );
            if ( legoccurrencesptr )
               addLeg ( path.legs[i]->tuple.connectingnode, tuple.depth, 
                        path.legs[i]->tuple.edgelabel, *legoccurrencesptr );
         }
      }
   }
   ...
   i = legindex + 1;
   addLeftLegs ( path, leg, i, leg.tuple.depth, 
                 path.edgelabels[leg.tuple.depth - 1], leftend, edgesize2 );
...
void PatternTree::addRightLegs ( Path &path, PathLeg &leg, int &i, 
                                 Depth olddepth, EdgeLabel lowestlabel, 
                                 int rightstart, int nodesize2 ) 
{
   int i2 = i + 1, k;
   LegOccurrencesPtr legoccurrencesptr;

   while ( i >= 0 && path.legs[i]->tuple.depth >= (unsigned int)rightstart )
   {
      ...
      if ( path.legs[i]->tuple.depth != olddepth ) 
      {
         for ( k = i + 1; k < i2; k++ ) 
         {
            ...
            {
               legoccurrencesptr = join ( leg.occurrences, 
                                          path.legs[k]->tuple.connectingnode, 
                                          path.legs[k]->occurrences );
               if ( legoccurrencesptr )
                  addLeg ( path.legs[k]->tuple.connectingnode, 
                           path.legs[k]->tuple.depth - nodesize2, 
                           path.legs[k]->tuple.edgelabel, *legoccurrencesptr );
            }
         }
         i2 = i + 1;
         olddepth = path.legs[i]->tuple.depth;
      ...
      if ( path.legs[i]->tuple.edgelabel == lowestlabel ) 
      {
         ...
         if ( legoccurrencesptr )
            addLeg ( path.legs[i]->tuple.connectingnode, 
                     path.legs[i]->tuple.depth - nodesize2, 
                     path.legs[i]->tuple.edgelabel, *legoccurrencesptr );
      }
      i--;
   }
...
int PatternTree::addRightLegs ( Path &path, PathLeg &leg, Tuple &tuple, 
                                unsigned int legindex, int rightstart, 
                                int nodesize2 ) 
{
   int i;
   LegOccurrencesPtr legoccurrencesptr = join ( leg.occurrences );
   ...
   if ( rootpathrelations.back () == 0 && tuple.depth != maxdepth )
      for ( int j = i + 1; (unsigned int)j < legindex; j++ ) 
      {
         LegOccurrencesPtr legoccurrencesptr = join ( leg.occurrences, 
                                                      path.legs[j]->tuple.connectingnode, 
                                                      path.legs[j]->occurrences );
         if ( legoccurrencesptr )
            addLeg ( path.legs[j]->tuple.connectingnode, tuple.depth, 
                     path.legs[j]->tuple.edgelabel, *legoccurrencesptr );
      }

   EdgeLabel lowestlabel = path.edgelabels[leg.tuple.depth];
   for ( unsigned int j = legindex + 1; j < path.legs.size () && path.legs[j]->tuple.depth == leg.tuple.depth; j++ ) 
   {
...
PatternTree::PatternTree ( Path &path, unsigned int legindex ) 
{
        ...
        leg.tuple.depth * 2 == path.edgelabels.size () ) 
   {
      ...
      if ( leftwalk == -1 || path.edgelabels[leftwalk] < path.edgelabels[rightwalk] ) 
      {
         ...
         if ( leg.tuple.depth <= (unsigned int)leftend ) 
         {
            ...
            for ( i = 0; (unsigned int)i < tuple.depth; i++ ) 
            {
            ...
            // add second path
            for ( i = rightstart, j = 0; (unsigned int)i < path.edgelabels.size (); i++, j++ ) 
            {
               vector_push_back ( Tuple, treetuples, tuple );
               tuple.label = path.edgelabels[i];
               tuple.depth = j;
      ...
      unsigned int targetdepth = path.nodelabels.size () - 1 - leg.tuple.depth;
      ...
      if ( targetdepth == path.edgelabels.size () - 1 ) 
      {
         ...
         int j2 = j;
         while ( j >= i && path.legs[j]->tuple.depth == targetdepth && 
                 path.legs[j]->tuple.edgelabel >= leg.tuple.edgelabel )
            j--;
         for ( int k = j + 1; k <= j2; k++ ) 
         {
            LegOccurrencesPtr legoccurrencesptr = join ( leg.occurrences, 
                                                         ...
                                                         path.legs[k]->occurrences );
            if ( legoccurrencesptr )
               addLeg ( path.legs[k]->tuple.connectingnode, tuple.depth, 
                        path.legs[k]->tuple.edgelabel, *legoccurrencesptr );
         }
         while ( j >= i && path.legs[j]->tuple.depth == targetdepth )
            j--;
...
PatternTree::PatternTree ( PatternTree &parenttree, unsigned int legindex ) 
{
   ...
   if ( !parenttree.rootpathstart && legindex < (unsigned int)parenttree.secondpathleg ) 
   {
   ...
   if ( parenttree.rootpathstart == 0 && legindex >= (unsigned int)parenttree.secondpathleg ) 
   {
   ...
   if ( (unsigned int)index == legindex ) 
   {
      LegOccurrencesPtr legoccurrencesptr = join ( leg.occurrences );
      if ( legoccurrencesptr )
   ...
   if ( rootpathstart == 0 ) 
   {
      ...
      while ( (unsigned int)index < parenttree.legs.size () ) 
      {
...
{
   unsigned int i = 0, j = nextpathstart;
   bool equal = true;

...
void GraphState::clearGraphState()
{
    nodes.clear();
    deletededges.clear();
}
...
void readDebug () {
  ...
  readint ( input );
    ...
    if ( dummy != nodessize ) {
      cerr << "(readDebug) Error reading input file - node number does not correspond to its position." << endl;
      exit ( 1 );
    }
    nodessize++;
...
void GraphState::init () 
{
   ...
   readDebug ();
#endif
   // If are in the second phase (have already found the best substructure),
   // read in the best substructure
   if (phase == 4)
   {
      cout << "Reading in best substructure information..." << endl;
      gbad.readBestSub();
      cout << "... finished reading best substructure information" << endl;
   }
   else
      cout << "initializating GraphState ..." << endl;
   edgessize = 0;
   closecount = 0;
...
{
   vector<LegOccurrence> nolegs;
   insertNode ( nodelabel, (short unsigned int) (-1), nolegs );
}
                             ...
                             vector<LegOccurrence> &elements) 
{
   ...
   insertNode ( tolabel, maxdegree, elements );
...
void GraphState::deleteEdge ( GSEdge &edge ) 
{
   ...
   if ( nodes[deletededge.tonode].edges.size () != (unsigned int)deletededge.postonode + 1 ) 
   {
      // edge2 not the last
   ...
   if ( nodes[deletededge.fromnode].edges.size () != (unsigned int)deletededge.posfromnode + 1 ) 
   {
...
void GraphState::reinsertEdge () 
{
   ...
   if ( (unsigned int)deletededge.postonode != edges1.size () - 1 ) 
   {
   ...
   if ( (unsigned int)deletededge.posfromnode != edges2.size () - 1 ) 
   {
      // reinsert at original location by swapping with the element at that position again
...
void GraphState::determineCycles ( unsigned int usedbit ) 
{
   ...
   while ( stacktop > 0 ) 
   {
      if ( nodes[nodestack[stacktop]].edges.size () <= (unsigned int)edgestack[stacktop] ) 
      {
      ...
      {
         if ( edge.tonode != (unsigned int)nodestack[stacktop-1] ) 
         {
            ...
            while ( had ) 
            {
               had = ( (unsigned int)nodestack[k] != edge.tonode );
...
//******************************************************************************
// NAME:    enumerateSpanning
//
// INPUTS:
//
// RETURN: returns true if lower found, otherwise false
//
// PURPOSE:
//
//******************************************************************************
...
{
   if ( (unsigned int)edgessize == nodes.size () - 1 ) 
   {
      // we have a tree
      if ( (unsigned int)closecount == closetuples->size () )
...
int GraphState::normalizetree () 
{
  ...
  // discover the leafs
   ...
   if ( (unsigned int)(queueend + onecnt) == nodes.size () ) 
   { // otherwise all nodes have been done already
   ...
   else 
   {
      ...
      GSNode &node = nodes[queue[queuebegin]];
      int i;
      for ( i = 0; node.edges[i].tonode != (unsigned int)queue[queuebegin-1]; i++ );
       ...
       lowesttreenr = lowestlabeltreenr;
        ...
        for ( int r = 0; r < depthnodessizes[depth+1]; r++ ) 
          ...
          if ( nodes_marker[parentid] == 1 &&
               ...
               nodes_edgelabel[nodeid] < (unsigned int)secondlowestlabel )
   ...
   for ( Depth depth = maxdepth - 1; ; depth-- ) 
   {
      ...
      for ( int i = 1; i < size; i++ ) 
      {
         ...
         unsigned int depthlabel = pathedgelabels[ismin1][depth];
         k--;
         do 
         {
            int node2 = dnodes[k];
   ...
   while ( stacksize != 0 ) 
   {
      ...
      if ( !bicenter || depths[stacksize] ) 
      {
         ...
         if (cmp > 0 || 
             (cmp == 0 && nodes_edgelabel[nodeid] < (*treetuples)[codewalk].label)) 
            return 2;
         if (cmp < 0 || 
             (cmp == 0 && nodes_edgelabel[nodeid] > (*treetuples)[codewalk].label))
            return 0;
   ...
   for ( unsigned int i = 1; i < deletededges.size (); i++ ) 
   {
      ...
      else 
      {
         if ( stacksize == btcodesize ) 
         {
            ...
            for ( int i = 0; i < ddsize; i++ ) 
            {
               ...
               int p = 0;
               unsigned int ni = nodesinbt[deletededges[i+1].fromnode];
               while ( ni != NONODE ) 
               {
                  p += btcode[ni];
                  ni = btparent[ni];
...
//
// NOTE:  Two issues with this code:  (1) it uses a bubblesort, which there is
//        probably a faster sorting algorithm than this, and (2) there is a
//        "goto" statement...
//

            // then sort it, while comparing with the current one.
            // stop when we discover that we can do better
            // algorithm: bubblesort ( instead of insertion sort)
